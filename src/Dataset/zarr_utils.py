...
def get_zarr_root(
    ...
    path = os.path.join(
        directory, f'{"mean_" if is_mean_file else ""}{start_year}_{end_year}.zarr'
    )
...
def create_variable(
    root: zarr.hierarchy.Group, var: VariableInfo, is_mean_file: bool = False
):
    if is_mean_file:
...
def create_mean_variable(root, var):
    ...
    if var.isSurfaceVar:
        ...
        variable = root.create_dataset(
            var.name,
            shape=(0, level_cnt, metrics_cnt),
            dtype=np.float64,
            chunks=(1, level_cnt, metrics_cnt),
            fill_value=-99999,
        )
...
def create_var(root: zarr.hierarchy.Group, var: VariableInfo):
    ...
    if var.isSurfaceVar:
        ...
        variable = root.create_dataset(
            var.name,
            shape=(0, level_cnt, lat_cnt, lon_cnt),
            dtype=np.float64,
            chunks=(12, level_cnt, lat_cnt, lon_cnt),
            fill_value=-99999,
        )
