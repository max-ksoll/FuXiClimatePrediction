...
from src.utils import (
    log_exec_time,
    get_nc_files,
    get_month_as_strings,
    get_years_as_strings,
)
...
@log_exec_time
def download_era5_atmospheric_data(
...
def map_nc_files(ds_download_path: os.PathLike | str) -> Dict[str, List[str]]:
    nc_files = set(
        map(lambda x: os.path.join(ds_download_path, x), get_nc_files(ds_download_path))
    )
...
if __name__ == "__main__":
    ...
    download_era5_surface_data(cds_client, "/data/sy_1958__ey_1958__tmp", 0, 0)
