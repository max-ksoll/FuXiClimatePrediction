...
from src.Dataset.dimensions import ORAS_VARIABLES, ERA_SURFACE_VARIABLES, ERA_ATMOS_VARIABLES, LAT, LON
...
class DataDownloader:
    ...
    def download(self, force_download: bool = False):
        downloader_logger.info(f"Downloading Data for {self.start_year} to {self.end_year}")
        if force_download:
            downloader_logger.info("Forcing download")
            downloader_logger.info("Deleting existing files in {self.download_path} and Directory itself")
    ...
    def map_nc_files(self):
        nc_files = set(map(lambda x: os.path.join(self.download_path, x), get_nc_files(self.download_path)))
    ...
    def download_era5_atmospheric_data(self):
        ...
        for idx in tqdm(range(len(decade_strings))):
            downloader_logger.debug(f"Downloading ERA5 Atmospheric for decade: {decades[idx]}")
            file_path = os.path.join(self.download_path, f"era5_atmospheric_{decades[idx]}.nc")
            self.cds_client.retrieve("reanalysis-era5-complete", {
                "class": "ea",
    ...
    def download_era5_surface_data(self):
        ...
        for year in utils.get_years_as_strings(start_year, end_year):
            downloader_logger.debug(f"Downloading ERA5 Surface Variables for year {year}")
            file_path = os.path.join(self.download_path, f'era5_surface_{start_year}.nc')
    ...
    def download_oras_data(self):
        ...
        file_path = os.path.join(self.download_path, f'oras_{start_year}_{end_year}.zip')
        self.cds_client.retrieve(
            'reanalysis-oras5',
            {
                'format': 'zip',
        ...
        self.oras_paths = list(map(lambda x: os.path.join(self.download_path, x), list(after - before)))
...
class DataConverter:
    ...
    def write_oras(self):
        ...
        for path in self.data_downloader.oras_paths:
            ...
            variable = list(filter(lambda x: x.cdf_name == variable_name, ORAS_VARIABLES))[0]
    ...
    def write_era_surface(self):
        ...
        for path in self.data_downloader.era_surface_paths:
            ...
            for variable in ERA_SURFACE_VARIABLES:
                ...
                for year in range(self.data_downloader.start_year, self.data_downloader.end_year + 1):
                    for month in range(1, 13):
                        converter_logger.debug(f"Writing ERA5 Surface for {variable} at time {year}/{month}")
